      ...
      if (data.ok) {
        ...
        if (data.count > 0) {
          $.each(data.results, function(i, item) {
            ...
            var img = $(
              "<img style= 'height:100%; width: 100%; max-height: 300px; max-width: 300px; object-fit:cover; padding-left: 9px; overflow: hide;' />"
            ).attr("src", item.Images[0].url_fullxfull);

            aTag.append(img);
            console.log(aTag);
            var abc = $("<div>")
              .addClass("col-3 float-left p-0")
              .attr("id", "imgDiv");
            abc.append(aTag);
